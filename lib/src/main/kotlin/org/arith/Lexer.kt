...
enum class TokenType {
    BANG, STAR, POW, PLUS, MINUS, SLASH, MOD, LEFT_PAREN, RIGHT_PAREN, COMMA, NUM, FUNCTION, EOF, ERROR
}
...
fun lex(lexer: Lexer): Pair<Token, Lexer> {
    ...
    val isUnaryMinus =
        (lexerCopy.source.length >= 2) &&
                (
                        (
                                lexer.lastTokenWasOp && head == '-' &&
                                        lexerCopy.source[1].isDigit()) || (head == '-' && lexerCopy.source[1].isDigit())
                        )
    ...
    val result = when (lexeme) {
        TOKEN_ERROR -> Token("Unknown lexeme: $head", tokenType, lexer.column, Precedence.NONE) to lexerCopy
        else ->
            Token(
                lexeme,
                tokenType,
                lexerCopy.column,
                if (isUnaryMinus) Precedence.UNARY else precedenceOf(
                    lexeme
                ),
                isLeftAssociative(tokenType, lexeme),
                if (tokenType == TokenType.FUNCTION) fromFunctionName(lexeme) else null
            ) to lexerCopy.copy(
...
private
fun isLeftAssociative(tokenType: TokenType, lexeme: String) = isOp(tokenType) && lexeme != "^"

private
fun parse(lexer: Lexer): Pair<String, TokenType> =
    when (lexer.source.first().lowercaseChar()) {
        ...
        else -> if (lexer.source.first().isDigit()) {
...
private
fun parseFunction(lexer: Lexer) = when (lexer.source.first()) {
    'a' ->
        listOf(
            "abs",
            "acos",
            ...
            "asin",
            "atan"
        ).firstOrNull { lexer.source.take(it.length) == it }?.let { it to TokenType.FUNCTION }

    'c' -> if (lexer.source.last() == 'h') {
...
private
fun fromFunctionName(functionName: String): ((List<Double>) -> Either<String, Double>)? =
    when (functionName) {
        ...
        "nroot" -> atLeastNArgs("nroot", 2) { args: List<Double> -> args.first().pow(1.0 / args[1]) }
        else -> null
    }

private
fun reciprocal(n: Double): Double = 1.0 / n

private
fun precedenceOf(lexeme: String): Precedence = when (lexeme) {
    "+", "-" -> Precedence.TERM
    "*", "/", "%" -> Precedence.FACTOR
    "!", "^" -> Precedence.FACTOR
...
private
fun skipWhiteSpace(lexer: Lexer): Lexer {
    val result =
        when (lexer.source.first()) {
            ' ', '\t' -> {
                lexer.source.dropWhile { it == ' ' || it == '\t' }.let {
                    lexer.copy(source = it, column = lexer.column + (lexer.source.length - it.length).absoluteValue)
                }
            }
...
private
fun isOp(tokenType: TokenType): Boolean = when (tokenType) {
    TokenType.BANG,
    TokenType.STAR,
    TokenType.POW,
    TokenType.PLUS,
