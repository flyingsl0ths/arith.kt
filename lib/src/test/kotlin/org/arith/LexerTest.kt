...
class LexerTest private constructor() {
    ...
    fun givenASourceContainingAllSingleCharsReturnsTheAppropriateTokens() {
        fun opPrecedence(lexeme: String): Precedence = when (lexeme) {
            "+", "-" -> Precedence.TERM
            "*", "/", "%" -> Precedence.FACTOR
            "!", "^" -> Precedence.FACTOR
        ...
        fun tokenTypeOfSingle(lexeme: String) = when (lexeme) {
            "!" -> TokenType.BANG
        ...
        "!*^+-%/(,)".forEach {
            ...
            assertEquals(
                token, Token(source, tokenTypeOfSingle(source), 0, prec, source != "^" && prec != Precedence.NONE)
            )
        }
    }
    ...
    fun givenAnKnownFunctionNameReturnsAnAppropriateToken() {
        listOf(
            ...
            assertEquals(token.copy(function = null), Token(it, TokenType.FUNCTION, 0, Precedence.NONE))
        }
    }
